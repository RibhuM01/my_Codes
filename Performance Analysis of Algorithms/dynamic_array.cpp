...
template<typename T> void DynamicArray<T> ::  insert_at_end(T data){
    if(this->arr == NULL){
        this->arr = new T[1];
        arr[0] = data;
        this->size=1;
...
template<typename T> void DynamicArray<T> :: insert_at_loc(int loc,T data){
        T *copyArray = new T[this->size+1];
