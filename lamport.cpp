	...
	while(true)
	{
		...
		if(is.str().size()!=0)
		{
			if(inputs[0]=="begin")
			{
				...
				if(inputs.size()!=3 || inputs[1]!="process" || inputs[2].size()!=2 || inputs[2][0]!='p' || !is_pid(inputs[2][1]))
				{
		...
		for(auto i=task.begin();i!=task.end();i++)
		{
			if(curr[i->first]!=i->second.size())
			{
				if(i->second.size()>0 && i->second[curr[i->first]][0]=="recv")
				{
					...
					if(i->second[curr[i->first]].size()!=3 || i->second[curr[i->first]][1].size()!=2 || i->second[curr[i->first]][1][0]!='p' || !is_pid(i->second[curr[i->first]][1][1]) || i->second[curr[i->first]][2][0]!='m' )
					{
